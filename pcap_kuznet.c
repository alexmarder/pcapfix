 ...
 *
 */
int check_header_kuznetzov(char *buffer, unsigned int size, unsigned int prior_ts, struct global_hdr_s *global_hdr,
                           struct packet_hdr_kuznet_s *hdr) {
...
//    }
    ...
    return (fix_pcap_packets_kuznetzov(pcap, pcap_fix, global_hdr, hdr_integ, writebuffer, writepos));
}
 ...
 *
 */
int fix_pcap_packets_kuznetzov(cfile_t *pcap, afile_t *pcap_fix, struct global_hdr_s global_hdr,
...
//        }
        ...
        if (bytes < sizeof(hdrbuffer)) {
            if (bytes > 0) {
                printf("[-] Not enough bytes left in file ==> SKIPPING bytes.\n");
                corrupted++;
        ...
        if (res != -1) {
            ...
            if (n < conint(packet_hdr.incl_len)) {
                ...
                if (verbose >= 1)
                    printf("[-] LAST PACKET MISMATCH (%u | %u | %u | %u)\n", conint(packet_hdr.ts_sec),
            ...
            if (check_header_kuznetzov(hdrbuffer, sizeof(hdrbuffer), conint(packet_hdr.ts_sec), &global_hdr,
                                       &next_packet_hdr) == -1) {
                ...
                for (nextpos = pos + sizeof(packet_hdr) + 1;
                    ...
                    if (verbose >= 2)
                        printf("[*] Trying Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n", (count + 1),
                    ...
                    res = check_header_kuznetzov(hdrbuffer, sizeof(hdrbuffer), last_correct_ts_sec, &global_hdr,
                                                 &next_packet_hdr);
                    if (res != -1) {

                        /* we found a proper header inside the packets body! */
                        if (verbose >= 1)
                            printf("[-] FOUND OVERLAPPING data of Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n",
            ...
            if (verbose >= 1)
                printf("[+] Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n", count, pos,
                       conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len),
                       conint(packet_hdr.orig_len));
            ...
            size_t totalsize = sizeof(packet_hdr) + conint(packet_hdr.incl_len);
            ...
            if (verbose >= 1)
                printf("[-] CORRUPTED Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n", count, pos,
                       conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len),
                       conint(packet_hdr.orig_len));
            ...
            for (nextpos = pos + sizeof(packet_hdr) + 1; (nextpos <= pos + sizeof(packet_hdr) + PCAP_MAX_SNAPLEN) ||
                ...
                if (res != -1) {
                    ...
                    if ((nextpos - (pos + sizeof(packet_hdr)) > PCAP_MAX_SNAPLEN) && (count == 1) && (deep_scan == 1)) {
                        ...
                        if (verbose >= 1)
                            printf("[+] (DEEP SCAN) FOUND FIRST Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n",
                        ...
                        if (verbose >= 1)
                            printf("[+] FOUND NEXT Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n",
                                   (count + 1), nextpos, conint(next_packet_hdr.ts_sec),
                                   conint(next_packet_hdr.ts_usec), conint(next_packet_hdr.incl_len),
                                   conint(next_packet_hdr.orig_len));
                        ...
                        if (res == 0) {
                            ...
                            /* print out information */
                            printf("[+] CORRECTED Packet #%u at position %" PRIu64 " (%u | %u | %u | %u).\n", count,
                                   pos, conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec),
                                   conint(packet_hdr.incl_len), conint(packet_hdr.orig_len));
