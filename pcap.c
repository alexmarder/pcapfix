 ...
 *
 */
int check_header(char *buffer, unsigned int size, unsigned int prior_ts, struct global_hdr_s *global_hdr,
    ...
    if (verbose >= VVVERBOSE) fprintf(stderr, "[-] Header plausibility check failed with error code %d\n", res);
...
//    fseeko(pcap, 0, SEEK_SET);
    ...
    if (verbose >= VERBOSE) fprintf(stderr, "[*] Analyzing Global Header...\n");
    bytes = cfile_read(&global_hdr, sizeof(global_hdr), 1, pcap);    /* read first bytes of input file into struct */
    if (bytes != 1) return -2;
    ...
    if (conshort(global_hdr.version_major) == 2) {    /* current major version is 2 */
        if (verbose > VERBOSE) fprintf(stderr, "[+] Major version number: %hu\n", conshort(global_hdr.version_major));
    } else {
        hdr_integ++;
        if (verbose > VERBOSE) fprintf(stderr, "[-] Major version number: %hu\n", conshort(global_hdr.version_major));
    ...
    if (conshort(global_hdr.version_minor) == 4) {    /* current minor version is 4 */
        if (verbose > VERBOSE) fprintf(stderr, "[+] Minor version number: %hu\n", conshort(global_hdr.version_minor));
    } else {
        hdr_integ++;
        if (verbose > VERBOSE) fprintf(stderr, "[-] Minor version number: %hu\n", conshort(global_hdr.version_minor));
    ...
    if (conshort(global_hdr.thiszone) ==
        0) {    /* in practise time stamps are always in GTM, so the correction is always zero */
        if (verbose > VERBOSE) fprintf(stderr, "[+] GTM to local correction: %d\n", conint(global_hdr.thiszone));
    } else {
        hdr_integ++;
        if (verbose > VERBOSE) fprintf(stderr, "[-] GTM to local correction: %d\n", conint(global_hdr.thiszone));
    ...
    if (conint(global_hdr.sigfigs) == 0) {    /* all capture tools set this to zero */
        if (verbose > VERBOSE) fprintf(stderr, "[+] Accuracy of timestamps: %u\n", conint(global_hdr.sigfigs));
    } else {
        hdr_integ++;
        if (verbose > VERBOSE) fprintf(stderr, "[-] Accuracy of timestamps: %u\n", conint(global_hdr.sigfigs));
    ...
    if ((conint(global_hdr.snaplen) > 0) &&
        (conint(global_hdr.snaplen) <= PCAP_MAX_SNAPLEN)) {    /* typically 262144 nowadays */
        if (verbose > VERBOSE) fprintf(stderr, "[+] Max packet length: %u\n", conint(global_hdr.snaplen));
    } else {
        hdr_integ++;
        if (verbose > VERBOSE) fprintf(stderr, "[-] Max packet length: %u\n", conint(global_hdr.snaplen));
    ...
    if (conint(global_hdr.network) <= 251) {    /* data link types are <= 251 */
        if (verbose > VERBOSE) fprintf(stderr, "[+] Data link type: %u\n", conint(global_hdr.network));
    } else {
        hdr_integ++;
        if (verbose > VERBOSE) fprintf(stderr, "[-] Data link type: %u\n", conint(global_hdr.network));
    ...
    if (data_link_type != -1) {
        if (verbose >= VERBOSE) fprintf(stderr, "[+] Changing data link type to %d.\n", data_link_type);
    ...
    if (hdr_integ == 0) { /* no field has been corrupted? --> header is intact */
        if (verbose >= VERBOSE) fprintf(stderr, "[+] The global pcap header seems to be fine!\n");
    } else if (hdr_integ >= 4) { /* there have been five or more (of seven) corrupted fields? --> header is missing */
        if (verbose >= VERBOSE) fprintf(stderr, "[-] The global pcap header seems to be missing ==> CORRECTED!\n");
        ...
        if (verbose >= VERBOSE) fprintf(stderr, "[-] The global pcap header seems to be corrupt! ==> CORRECTED\n");
    }
    ...
    return (fix_pcap_packets(pcap, pcap_fix, global_hdr, hdr_integ, writebuffer, writepos));
}
 ...
 *
 */
int fix_pcap_packets(cfile_t *pcap, afile_t *pcap_fix, struct global_hdr_s global_hdr,
                     unsigned short hdr_integ, char *writebuffer, uint64_t writepos) {
...
//    unsigned int step = 1;            /* step counter for progress bar */
    ...
    if (verbose >= VERBOSE) fprintf(stderr, "[*] Analyzing packets...\n");
    ...
    pos = cfile_tello(pcap);

    /* loop the pcap files packets until pos has reached end of file */
    for (count = 1;; count++) {
...
//        printf("Iteration: %ul\n", count);
        ...
        bytes = cfile_read(hdrbuffer, 1, sizeof(hdrbuffer), pcap);
        if (bytes < sizeof(hdrbuffer)) {
            if (bytes > 0) {
                if (verbose >= VERBOSE) fprintf(stderr, "[-] Not enough bytes left in file ==> SKIPPING bytes.\n");
                corrupted++;

                /* at least one packet has been found?? */
                if (count == 1) {
                    if (verbose >= VERBOSE) fprintf(stderr, "[-] No packet has been found!\n");
                    return (-2);
                }
            }
...
//            printf("Breaking\n");
        ...
        if (res != -1) {
            ...
            pos += res;
            cfile_seeko(pcap, pos + sizeof(packet_hdr), SEEK_SET);

            /* try to read the packet body AND check if there are still at least 16 bytes left for the next pcap packet header */
            uint64_t n = cfile_read(&buffer, 1, conint(packet_hdr.incl_len), pcap);
            if (n < conint(packet_hdr.incl_len)) {
                ...
                if (verbose >= VVERBOSE)
                    if (verbose >= VERBOSE)
                        fprintf(stderr, "[-] LAST PACKET MISMATCH (%u | %u | %u | %u)\n", conint(packet_hdr.ts_sec),
                                conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len), conint(packet_hdr.orig_len));

                /* correct the packets included length field to match the end of file */
                packet_hdr.incl_len = conint(n);

                /* the original length must not be smaller than the included length */
                if (conint(packet_hdr.incl_len) > conint(packet_hdr.orig_len))
                    packet_hdr.orig_len = packet_hdr.incl_len;

                /* print out information */
                if (verbose >= VERBOSE) fprintf(stderr, "[+] CORRECTED Packet #%u at position %"
                PRIu64
                " (%u | %u | %u | %u).\n", count, pos,
                        conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len),
                        conint(packet_hdr.orig_len));
                corrupted++;
            }
            ...
            bytes = cfile_read(hdrbuffer, sizeof(hdrbuffer), 1, pcap);

            /* check if next packets header looks proper */
            if (check_header(hdrbuffer, sizeof(hdrbuffer), conint(packet_hdr.ts_sec), &global_hdr, &next_packet_hdr) ==
                -1) {
                ...
                for (nextpos = pos + sizeof(packet_hdr) + 1;
                     (nextpos < pos + sizeof(packet_hdr) + conint(packet_hdr.incl_len) + 2 * sizeof(packet_hdr)) ||
                     (count == 1 && deep_scan == 1); nextpos++) {

                    /* read the possible next packets header */
                    cfile_seeko(pcap, nextpos, SEEK_SET);
                    bytes = cfile_read(hdrbuffer, sizeof(hdrbuffer), 1, pcap);
                    ...
                    if (verbose >= VVVERBOSE)
                        fprintf(stderr, "[*] Trying Packet #%u at position %"
                    PRIu64
                    " (%u | %u | %u | %u).\n", (count + 1),
                            nextpos, conint(next_packet_hdr.ts_sec), conint(next_packet_hdr.ts_usec),
                            conint(next_packet_hdr.incl_len), conint(next_packet_hdr.orig_len));

                    /* check the header for plausibility */
                    res = check_header(hdrbuffer, sizeof(hdrbuffer), last_correct_ts_sec, &global_hdr,
                                       &next_packet_hdr);
                    if (res != -1) {

                        /* we found a proper header inside the packets body! */
                        if (verbose >= VVERBOSE)
                            fprintf(stderr, "[-] FOUND OVERLAPPING data of Packet #%u at position %"
                        PRIu64
                        " (%u | %u | %u | %u).\n",
                                (count + 1), nextpos, conint(next_packet_hdr.ts_sec),
                                conint(next_packet_hdr.ts_usec), conint(next_packet_hdr.incl_len),
                                conint(next_packet_hdr.orig_len));
                        ...
                        packet_hdr.incl_len = conint(nextpos - (pos + sizeof(packet_hdr)) +
                                                     res);    /* also include ascii corruption offset (res) */
                        packet_hdr.orig_len = packet_hdr.incl_len;
            ...
            cfile_seeko(pcap, pos + sizeof(packet_hdr) + conint(packet_hdr.incl_len), SEEK_SET);

            /* we found a correct packet (and aligned it maybe) */
            if (verbose >= VVERBOSE)
                fprintf(stderr, "[+] Packet #%u at position %"
            PRIu64
            " (%u | %u | %u | %u).\n", count, pos,
                    conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len),
                    conint(packet_hdr.orig_len));
            ...
            uint64_t totalsize = sizeof(packet_hdr) + conint(packet_hdr.incl_len);
            if (writepos + totalsize > 1024000) {
                bytes = afile_write(writebuffer, writepos, 1, pcap_fix);
                writepos = 0;
            }
            ...
            if (verbose >= VVERBOSE)
                fprintf(stderr, "[-] CORRUPTED Packet #%u at position %"
            PRIu64
            " (%u | %u | %u | %u).\n", count, pos,
                    conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len),
                    conint(packet_hdr.orig_len));
            ...
            for (nextpos = pos + sizeof(packet_hdr) + 1; (nextpos <= pos + sizeof(packet_hdr) + PCAP_MAX_SNAPLEN) ||
                                                         (count == 1 && deep_scan == 1); nextpos++) {

                /* read the possible next packets header */
                cfile_seeko(pcap, nextpos, SEEK_SET);
                uint64_t n = cfile_read(hdrbuffer, 1, sizeof(hdrbuffer), pcap);
                if (n < sizeof(hdrbuffer)) {
                    ...
                    if (verbose >= VERBOSE)
                        fprintf(stderr, "[*] End of file reached. Aligning last packet.\n");

                    /* check last packet for plausiblilty */
                    res = check_header(hdrbuffer, sizeof(hdrbuffer), last_correct_ts_sec, &global_hdr, &packet_hdr);
                    if (res != 0) {
                        if (verbose >= VERBOSE)
                            fprintf(stderr, "[-] Cannot align last packet, because it is broken.\n");
                        corrupted++;
                        count--;
                        break;
                    }
                    ...
                    cfile_seeko(pcap, pos + sizeof(packet_hdr), SEEK_SET);
                    bytes = cfile_read(&buffer, conint(packet_hdr.incl_len), 1, pcap);
                    ...
                    /* print out information */
                    if (verbose >= VERBOSE)
                        fprintf(stderr, "[+] CORRECTED LAST Packet #%u at position %"
                    PRIu64
                    " (%u | %u | %u | %u).\n", count, pos,
                            conint(packet_hdr.ts_sec), conint(packet_hdr.ts_usec), conint(packet_hdr.incl_len),
                            conint(packet_hdr.orig_len));
                ...
                PRIu64
                " (%u | %u | %u | %u).\n", (count + 1), nextpos,
                        conint(next_packet_hdr.ts_sec), conint(next_packet_hdr.ts_usec),
                        conint(next_packet_hdr.incl_len), conint(next_packet_hdr.orig_len));
                ...
                if (res != -1) {
                    ...
                    if ((nextpos - (pos + sizeof(packet_hdr)) > PCAP_MAX_SNAPLEN) && (count == 1) && (deep_scan == 1)) {
                        ...
                        if (verbose >= VVERBOSE)
                            fprintf(stderr, "[+] (DEEP SCAN) FOUND FIRST Packet #%u at position %"
                        PRIu64
                        " (%u | %u | %u | %u).\n",
                                count, nextpos, conint(next_packet_hdr.ts_sec), conint(next_packet_hdr.ts_usec),
                                conint(next_packet_hdr.incl_len), conint(next_packet_hdr.orig_len));
                        ...
                        if (verbose >= VVERBOSE)
                            fprintf(stderr, "[+] FOUND NEXT Packet #%u at position %"
                        PRIu64
                        " (%u | %u | %u | %u).\n",
                                (count + 1), nextpos, conint(next_packet_hdr.ts_sec),
                                conint(next_packet_hdr.ts_usec), conint(next_packet_hdr.incl_len),
                        ...
                        if (res == 0) {
                            ...
                            /* print out information */
                            if (verbose >= VERBOSE)
                                fprintf(stderr, "[+] CORRECTED Packet #%u at position %"
                            PRIu64
                            " (%u | %u | %u | %u).\n", count, pos, conint(packet_hdr.ts_sec), conint(
                                    packet_hdr.ts_usec), conint(packet_hdr.incl_len), conint(packet_hdr.orig_len));
            ...
            if ((count == 1 && deep_scan == 0) && (nextpos > pos + sizeof(packet_hdr) + PCAP_MAX_SNAPLEN)) {
                ...
                if (verbose >= VVERBOSE) fprintf(stderr, "[-] Cannot align corrupted packet! \n");
                break;
            }
            ...
            if (deep_scan == 0 && (nextpos > pos + sizeof(packet_hdr) + PCAP_MAX_SNAPLEN)) {
                if (verbose >= VVERBOSE)
                    fprintf(stderr, "[-] No next packet found within max packet range --> SKIPPING!\n");
...
//    if (verbose == 0) { print_progress(pos, filesize); }
    ...
    if (end == 0) { /* no ==> data missing == FAILED */
        if (verbose >= VERBOSE)
            fprintf(stderr, "[-] Reached EOF without finding more packets!\n");
    ...
    if ((hdr_integ == 0) && (corrupted == 0)) { /* check allover failure / integrity count and corrupted counter */
        ...
        if (verbose >= VERBOSE) fprintf(stderr, "[*] Wrote %u packets to file.\n", count - 1);
